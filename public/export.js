...
(function (root, factory) {
  ...
  function toMarkdown({ meta, rows = [], notes = [], noteFiles = {} }) {
    ...
    L.push('');
    /*
     * A SIMULATED SIT SAYS SO FIRST, BEFORE ANYTHING THAT LOOKS LIKE A RESULT.
     *
     * The simulator (simdevice.js, `?sim=1`) exists to tell a broken build from an unconnected
     * headband, and its one unacceptable failure is a fabricated sit being read later as a real one.
     * The rows are identical in shape to real ones, so nothing about the data can be used to tell
     * afterwards — the marker has to be written at export time, and it has to be impossible to miss.
     * Hence three independent places: the filename, this banner, and a SIMULATED.txt in the archive.
     * Any one of them can be lost to a copy, a rename or a reader who skips the report.
     */
    if (meta.simulated) {
      L.push('> ## ⚠ SIMULATED DATA — NOT A RECORDING OF ANYONE');
      L.push('>');
      L.push('> This sit was produced by the built-in signal simulator with no headband connected.'
        + ' Every number below is fabricated from a scripted waveform. It is useful for checking that'
        + ' the app works and for nothing else. Do not pool it with real sits: there is no way to'
        + ' separate it again afterwards.');
      L.push('');
    }
  ...
  function buildFiles(session, audioBytesById = {}) {
    ...
    /* A file whose NAME is the warning, so it shows up in a directory listing and in the zip's own
       table of contents without anything being opened or read. */
    if (meta.simulated) {
      files.push({
        name: 'SIMULATED.txt',
        bytes: utf8([
          'THIS ARCHIVE CONTAINS SIMULATED DATA.',
          '',
          'No headband was connected. The EEG, the metrics and every score in session.md were',
          'generated by the app\'s built-in signal simulator (public/simdevice.js), which produces a',
          'scripted waveform: a slow arc from a beta-dominant "restless" state to an alpha-dominant',
          '"settled" one and back, on a 90-second cycle.',
          '',
          'It is a measurement of nobody. It exists so that the app can be checked end to end without',
          'hardware. Do NOT include it in any analysis, comparison or pooled dataset — the rows have',
          'the same shape as real ones, so once mixed in they cannot be identified again.',
        ].join('\n')),
      });
    }
    ...
    files.push({
      ...
      bytes: utf8([
        ...
        '',
        'WHICH COLUMNS CAN BE COMPARED BETWEEN SITS, which is not all of them:',
        '',
        '  CAN BE.  breathPerMin, hrBpm, hrvMs are physical quantities on fixed scales — a',
        '           breath rate of 6/min means the same thing in every recording. The four',
        "           `levels` are each channel's ALPHA SHARE of alpha+beta power, a bounded",
        '           ratio, so they are comparable too (subject to how well the electrode was',
        '           sitting).',
        '',
        '  CANNOT.  calm, focus, thinking, drowsy, equanimity and the rest are normalised',
        '           WITHIN the sit. The app rescales each one against its own recent range on',
        '           a roughly 250-second baseline, so "calm 0.32" means "low compared with the',
        '           last few minutes of THIS recording" — not "low compared with your other',
        '           sits". Pooling them across sessions compares each sit against itself and',
        '           reports the result as a difference between sits. To compare sits on these,',
        '           recompute them from the raw eeg-ch*.f32 files against a fixed scale.',
        '',
        'THE SCALE IS FIXED AFTER TWO MINUTES. Those normalised columns learn their',
        'baseline from the first two minutes of usable signal in the sit and then hold',
        'it, so within one recording a rise is a real rise rather than the baseline',
        'sliding. That makes them readable over time WITHIN a sit. It does not make',
        'them comparable BETWEEN sits: two sits that started differently have',
        'different zeroes.',
        '',
        'FIVE COLUMNS THAT SAY HOW GOOD THE MEASUREMENT WAS, not what it measured:',
        '',
        '  breathPhase   -1 fully exhaled, +1 fully inhaled, 0 mid-breath. This is the',
        '                only column that can express a HELD breath: held at the top of',
        '                an inhale the chest stays expanded and this stays positive,',
        '                while breathPerMin looks the same as a dead sensor because a',
        '                held breath has no respiratory modulation at all.',
        '  breathRising  1 inhaling, 0 exhaling, blank unknown.',
        '  breathSource  chest | rsa | ppg. NOT interchangeable: chest is measured',
        '                movement, rsa is inferred from beat timing and trails the real',
        '                breath by about a second, ppg is inferred through a temple.',
        '                Pooling them mixes a measurement with an inference.',
  ...
  function archiveName(meta) {
    /* The prefix is the marker that survives everything. A file can be renamed, a report can go
       unread, and a zip can be unpacked and its extra files ignored — but the default name is what
       ends up in a folder of sits, and "SIMULATED-" sorts and reads unmistakably next to the real
       ones. Belt and braces on purpose: see the note in toMarkdown about why one marker is not
       enough. */
    return `${meta && meta.simulated ? 'SIMULATED-' : ''}meditation-${stamp(new Date(meta.startedAt))}.zip`;
  }
