...
The ten modes, in cycle order.

**Ribbon's chop had two causes, both worth knowing.** The array of samples scrolls, so selecting every
third by array index re-picks a *different* third on every push — and a Catmull-Rom curve through a
different control set is a different curve, so the whole ribbon re-shaped 4×/second. Control points are
now chosen by a counter that belongs to the sample (`seq`), so a sample is a control point for its whole
life or never. The shimmer was also keyed to screen `x`, which meant the pattern was fixed in space while
the ribbon slid through it; it is keyed to `seq` now, so the ripple travels with the surface.

| Mode | What it is | What drives it |
|---|---|---|
| **Lumen** | *The default, and what Meditate opens on.* Five large overlapping lobes of coloured light, elliptical and slowly turning, drifting through each other. No axis, no head, no history — nothing in it reads as a chart. | Coherence is **focus**: high focus draws the lobes toward one form, low focus scatters them. Hue is **calm against thinking** — amber and gold when settled, violet and indigo when busy — and the ground warms with it, since a cool ground tints a dim warm field toward muddy violet. Drift speed is **thinking**; breadth and brightness are **calm**; the pulse is your **measured breath**, or a fixed 10s pacer when nothing is measuring one, and the key says which. Nothing here touches the sample buffer: every position is a continuous function of elapsed time and the data only sets eased parameters, so it cannot tick at the sample rate. It also has **no memory** — you cannot see how the sit went, which is what Iris and Flow are for. |
| **Ribbon** | *Kept, but no longer the default* — reported as "too similar to flow ... really choppy too", and both were true for one reason: its geometry IS the sample buffer, which makes it a chart however it is drawn and makes it change four times a second however it is smoothed. Two specific causes of the chop are fixed (see below), so it is a better version of what it is. |
| ~~**Ribbon**~~ | Four silk ribbons flowing left to right — Calm, Focus, Thinking, Drowsy — over a warm bloom at the centre that breathes. The right-hand end is now; the left edge is a minute ago, dissolving. | Every quality of it is a reading and none of it is decoration. **Height** is the score. **Thickness** is the same score again — that redundancy is why it works as something to sit in front of, because with your eyes half closed you cannot read four positions but you can see which band is broad. **Brightness** is the score a third time. The **shimmer** at the live end is Thinking, and it is fenced to the newest third so the record behind you stays exactly still. The **bloom**'s width is Calm and its slow pulse is your measured breath — or, when nothing is measuring one, a fixed 10-second pacer, and the key says which. Height comes from a **constant** transform of the score (`expandSoft(v, 0.28, 0.82)`), so unlike every auto-ranged trace here two different sits are directly comparable by eye, and drift is not fixed but impossible: there is no state to drift. A metric with no inputs leaves a **gap**, because a ribbon thinning to nothing would read as "calm collapsed" when it means "the headband stopped reading". |
...
| **Flow** | A live trace, thin and sharp where it's being written, dissolving as it ages. "Now" sits at ~74% across, history trailing left, scrolling continuously between samples rather than jumping four times a second. Each line is **named in a pill at its own head**, at the line's height. | Plots **any mix** of the eleven series in `VizCore.FLOW_SERIES` — four electrodes, four composites, breath, heart rate and HRV — chosen with the picker in the live feed. Every series carries its **own fixed axis**, which is what makes mixing an alpha share with a 0–100 score and a heart rate meaningful; the key says so, since a reader assuming one shared scale would misread every crossing. Pills at the same value are pushed apart and get a leader back to their dot. |
